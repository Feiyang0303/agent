...
2. 在章节中寻找与主题相关的、能引发听众共鸣的代表性案例，这些案例应能清晰地说明或支撑章节的主要论点。案例数量不能大于3个。
3. 为每个案例提供一个核心主题，核心主题应准确反映案例的主要议题或作者试图传达的主要思想，具有抽象性、深刻性。
4. 为每个案例提供详细的描述，确保案例发展的顺序是清晰的，并且因果关系明确。
----格式要求----
请按以下格式将案例以JSON格式输出，不要包含任何格式信息。
...
"""
    prompt_text = prompt_text.replace('\n\n', '\n')
    res = ollama.generate(model="qwen2.5:32b-instruct", prompt=prompt_text, system=prompt_sys, keep_alive='1h',
                          options={"num_ctx": 30720},format='json')
    result = res['response']
